...
impl Floor {
                ...
                for segment in &hallway.segments {
                    match segment.direction {
                        Direction::Up | Direction::Down => {
                            if location.x == segment.initial_location.x && (segment.initial_location.y..segment.initial_location.y + segment.length).contains(&location.y) {
                                return true;
                            }
                        },
                        _ => {
                            if location.y == segment.initial_location.y && (segment.initial_location.x..segment.initial_location.x + segment.length).contains(&location.x) {
                                return true;
                            }
                        }
                    }
        ...
        if let Some(rooms) = &self.rooms {
            for room in rooms {
                if starting_point.x == target_point.x
                    && (room.rel_loc.x..room.rel_loc.x + room.dimensions.x).contains(&starting_point.x) 
                    && (room.rel_loc.y <= starting_point.y.max(target_point.y)
                        && target_point.y.min(starting_point.y) <= room.rel_loc.y + room.dimensions.y
                    ) {
                    if starting_point.y < target_point.y {
                        return Some(Vec2::new(starting_point.x, room.rel_loc.y - 2));
                    } else {
                        return Some(Vec2::new(starting_point.y, room.rel_loc.y + room.dimensions.y + 2));
                    }
                } else if starting_point.y == target_point.y
                    && (room.rel_loc.y..room.rel_loc.y + room.dimensions.y).contains(&starting_point.y)
                    && (room.rel_loc.x <= starting_point.x.max(target_point.x)
                        && target_point.x.min(starting_point.x) <= room.rel_loc.x + room.dimensions.x
                    ) {
                    if starting_point.x < target_point.x {
                        return Some(Vec2::new(room.rel_loc.x - 2, starting_point.y));
                    } else {
                        return Some(Vec2::new(room.rel_loc.x + room.dimensions.x + 2, starting_point.y));
                    }
                }
            }
            return None;
    ...
    fn get_room_adjacent_tile(&self, room: &Room) -> Vec2 {
        ...
        return if [Direction::Up, Direction::Down].contains(&direction) {
            Vec2::new(fastrand::u32(room.rel_loc.x..=room.rel_loc.x + room.dimensions.x), if direction == Direction::Down {
                room.rel_loc.y - 1
            } else {
                room.rel_loc.y + room.dimensions.y + 1
            })
        } else {
            Vec2::new(if direction == Direction::Left {
                room.rel_loc.x - 1
            } else {
            ...
            if let Some(connections) = &self.connections {
                for connection in connections {
                    ...
                    while current_point != end_point {
                        println!("{},{} -> {},{}", current_point.x, current_point.y, end_point.x, end_point.y);
                        let mut target_point = if i32::abs(current_point.x as i32 - end_point.x as i32) > i32::abs(current_point.y as i32 - end_point.y as i32) || last_dir.map_or(false, |true_if_horizontal| true_if_horizontal == false) {
                            Vec2::new(end_point.x, current_point.y)
                        } else {
                            Vec2::new(current_point.x, end_point.y)
                        };
                        
                        println!("Initial Target Point: {}, {}", target_point.x, target_point.y);

                        if let Some(early_terminating_point) = self.segment_intersects_room(current_point, target_point) {
                           target_point = early_terminating_point; 
                        }

                        println!("Updated Target Point: {}, {}", target_point.x, target_point.y);
